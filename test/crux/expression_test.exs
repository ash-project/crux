...
defmodule Crux.ExpressionTest do
  ...
  describe inspect(&Expression.in_cnf?/1) do
    test "recognizes valid CNF" do
      ...
      assert Expression.in_cnf?(b(not :a))
      assert Expression.in_cnf?(b(:a or :b))
      assert Expression.in_cnf?(b(:a and :b))
      assert Expression.in_cnf?(b((:a or :b) and :c))
      assert Expression.in_cnf?(b((:a or :b) and (:c or :d)))
      assert Expression.in_cnf?(b(not :a or :b))
      assert Expression.in_cnf?(true)
      assert Expression.in_cnf?(false)
    end

    test "rejects invalid CNF" do
      refute Expression.in_cnf?(b(:a or (:b and :c)))
      refute Expression.in_cnf?(b(not (:a and :b)))
      refute Expression.in_cnf?(b(not (:a or :b)))
      refute Expression.in_cnf?(b(not not :a))
    end
  end
